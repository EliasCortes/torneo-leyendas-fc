...
def generar_torneo(nombre_archivo="torneo_nuevo.json"):
    
    tournament = Tournament("Torneo generado")

    # jugadores fijos (luego los podrás hacer dinámicos)
    player1 = HumanPlayer("Elias")
    player2 = HumanPlayer("Alvaro")

    tournament.add_human_player(player1)
    tournament.add_human_player(player2)

    # ruletas
    team_roulette = Roulette("Ruleta Equipos", TOURNAMENT_TEAMS.copy(), True)
    diamond_roulette = Roulette("Ruleta Diamante", DIAMOND_LEGENDS.copy(), True)
    gold_roulette = Roulette("Ruleta Oro", GOLD_LEGENDS.copy(), True)
    silver_roulette = Roulette("Ruleta Plata", SILVER_LEGENDS.copy(), True)
    bronze_roulette = Roulette("Ruleta Bronce", BRONZE_LEGENDS.copy(), True)
    option_roulette = Roulette("Ruleta Opciones", OPTION_ROULETTE.copy(), False)
    ...
    # guardar
    save_tournament_to_json(tournament, nombre_archivo)

    return nombre_archivo


# =========================
# MODO CONSOLA (NO ROMPE FLASK)
# =========================
...
if __name__ == "__main__":
    ...
    if modo == "2":
        ...
        # ruletas
        team_roulette = Roulette("Ruleta de Equipos", TOURNAMENT_TEAMS.copy(), True)
        diamond_roulette = Roulette("Ruleta Diamante", DIAMOND_LEGENDS.copy(), True)
        gold_roulette = Roulette("Ruleta Oro", GOLD_LEGENDS.copy(), True)
        silver_roulette = Roulette("Ruleta Plata", SILVER_LEGENDS.copy(), True)
        bronze_roulette = Roulette("Ruleta Bronce", BRONZE_LEGENDS.copy(), True)
        option_roulette = Roulette("Ruleta de Opciones", OPTION_ROULETTE.copy(), False)
